...
This documentation is intended for project users and administrators. It explains in a simple way how to use and manage the web infrastructure set up.
...
* **First step:**  `cp env.example .env` and modif .env with your informations (or not use but see the name of the user). 
...
* **WordPress site:** Go to `https://localhost`.
* **WordPress Administration:** Go to `https://localhost/wp-admin/` to manage the website.
* **Database Administration (phpMyAdmin):** Go to `https://localhost/phpmyadmin/` (don't forget the `/` at the end).
...
* These passwords are securely stored in local files located in the `secrets/` folder at the root of the project:
  * `secrets/db_root_password.txt`: The password of the supreme database administrator.
